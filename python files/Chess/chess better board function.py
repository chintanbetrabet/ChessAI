...
PAwn kills even head on but when checked, minimax sets to zero
...
class Board_State():
    ...
    def board_control(self):
        ...
        control=0
        for c in best_pos:
            for b in self.univ_leg:
                if b==c:
                    control+=1
        
        
        #control=len(self.univ_leg)
        #if control>0:
            #self.display_board()
            #print self.univ_leg
    ...
    def evaln(self):
        ...
        if KiC!=0:
            print "check mate"
            return -99999999999
        return self.board_control()*(-10)+QuC*9+KnC*4+RoC*5+BiC*3+PnC+KiC*999
    ...
    def bishop_move(self,cont=1,color='B',move=1,level=0):
        bishop_count=0
        bishops=[]
    ...
    def bishoplegal(self,bishops,move,level,pos_in=-1,player='C'):
        ...
        return 1
     #('W',2,level)           
    def knight_move(self,cont=1,color='B',move=1,level=0):
        knight_count=0
        knights=[]
    ...
    def knightlegal(self,knights,move,level,pos_in=-1,player='C'):
     ...
     #('W',2,level)       
    def rook_move(self,cont=1,color='B',move=1,level=0):
        rook_count=0
        rooks=[]
    ...
    def rooklegal(self,rooks,move,level,pos_in=-1,player='C'):
        ...
        if player!='C':
            ...
            return -1
        return 1
    def queen_move(self,cont=1,color='B',move=1,level=0):
        queen_count=0
        queens=[]
        if color=='B':
    ...
    def queenlegal(self,queens,move=1,level=0,pos_in=-1,player='C'):
        for qn in queens:
            ...
            if check1>0 or check2>0:
                ...
                return -1
    def king_move(self,cont=1,color='B',move=1,level=0):
        king_count=0
        kings=[]
        if color=='B':
    ...
    def pawn_move(self,cont=1,color='B',move=1,level=0):
        pawn_count=0
        pawns=[]
        if color=='B':
    ...
    def move_piece(self,pos,pos2,piece,move=1,level=0,pos_in=-1):
        ...
        if Board1.Set[pos2].Piece.Color==opp:
            ...
            if (Board1.Set[pos2].Piece.Type!='Ki'and pos_in!=-1) or pos_in==-1:
        ...
        if pos_in==pos2 :
            ...
            if move==2 :
                #print piece.Type
                '''Board1.display_board()
                print "val=%d control=%d"%(Board1.evaln(),Board1.board_control())
                raw_input()
                '''
...
def make_move1(board,ismove2=1):
    global move1
    global move_jump
    move1=move1[0:0]
    move_jump=move_jump[0:0]
    move=1
    level=0
    t1=time.clock()
    board.bishop_move()
    ...
    board.king_move()
    i=0
    print "move1 time=%f"%(time.clock()-t1)
    t1=time.clock()
    for x in move1:
        make_move2(x,i)
...
def make_move2(board,level):
    ...
    board.bishop_move(1,'W',2,level)
    board.queen_move(1,'W',2,level)
    board.rook_move(1,'W',2,level)
    board.knight_move(1,'W',2,level)
    board.pawn_move(1,'W',2,level)
    board.king_move(1,'W',2,level)
