...
def make_move1(board,color,player='C'):
    print
    global move1
    move1=move1[0:0]
    move=1
...
def Play_user(board,color):
    ...
    if lis_res==1:
        ...
        os.system('cls')
        return board1,-1
    board1.Weight[board1.Set[pos2].Piece.Type]+=board1.alpha*(board1.evaln(0,color)-board1.getScore(color))
    #print "BC:%d"%board.bcWeight
    board1.bcWeight+=board1.alpha*(board1.evaln(0,color)-board1.getScore(color))*board1.control[color]
...
def Play1(player,board,color) :
    ...
    board.Weight[board.LastMoved]+=board.alpha*(board.getScore(color)-board.minimaxVal)
    #print "BC:%d"%board.bcWeight
    board.bcWeight+=board.alpha*(board.getScore(color)-board.minimaxVal)*board.control[color]
    return board  
...
def game():
    ...
    while check_mate==0 and i<500:
        ...
        print "%sMoved %s from %d to %d "%(color,board.LastMoved,board.LastPos,board.NextPos)
