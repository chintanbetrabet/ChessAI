...
class Board_State():
    ...
    def bishoplegal(self,bishops,move,level,pos_in=-1,player='C'):
        ...
        if player!='C':
            #print "legal moves for Bishop are to"
            #print leg_pos
            ...
            for check in leg_pos:
                ...
                if pos_in==check:
                    return 1
            if bishops[0].Type=='Bi':
                print "illegal move for Bishop at pos_in=%d"%pos_in    
            return -1
        else :
             for check in leg_pos:
                self.univ_leg.update({check})
...
def Play_CPU(board,color):
    ...
    print "CPU took " + str(time.clock()-t1)
    raw_input()
    return move1[pos]
...
def Play_user(board,color):
    os.system('cls')
    board.display_board()
    pos=(raw_input("Enter Start position"))
    while pos=="":
        pos=(raw_input("Enter Start position"))
    pos=int(pos)
    print pos
    level=0
    pos2=(raw_input("Enter Final position"))
    while pos2=="":
        pos2=(raw_input("Enter Final position"))
    pos2=int(pos2)
    print pos2
