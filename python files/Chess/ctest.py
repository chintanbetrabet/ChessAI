...
def minimax(lis):
    ...
    while mover < len(lis):
        ...
        run+=1
        if lis[mover][0]>check_level:
            check_level=lis[mover][0]
            print "hi"
            ...
            local_min=global_min
        if lis[mover][0]==check_level:
            if lis[mover][1].evaln()<minm:
                    print "hi1"
                    mover=move_jump[check_level]
                    check_level+=1
                    #raw_input()
                    min_lis.append(lis[mover][1].evaln())
            else:
                print "yo1"
                if lis[mover][1].evaln()<local_min:
                    local_min=lis[mover][1].evaln()
                mover+=1
    ...
    for i in range(len(min_lis)):
        if min_lis[i]>minm:
            ...
            minm=min_lis[i]
    t2=time.clock()
    print "used%d tries and time=%d"%(run,t2-t1)
