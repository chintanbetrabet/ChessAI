...
class Map(SearchProblem):
    ...
    def find_curr(self):
        ...
        for i in range(self.xsize*self.ysize):
            if self.puzzle[i].val=='s':
                #print "found"
                pos=self.puzzle[i].x*20+self.puzzle[i].y
                return pos
        print "missing"
        #raw_input()
    ...
    def is_up(self):
        ...
        y=pos%self.ysize
        if ( x >0 ):
            pos2=(x-1)*self.ysize+(y)
            if self.puzzle[pos2].val !='#':
                return True
            else :
                return False
        else :
    ...
    def do_move(self,move):
        ...
        self.puzzle[pos].val=self.puzzle[pos2].val
        self.puzzle[pos2].val=temp
    ...
    def heuristic(self):
        ...
        if len(self.treasures)==0:
            ...
            for tre in self.treasures:
                ...
                dis.append(add)
        return min(dis)*len(self.treasures)
